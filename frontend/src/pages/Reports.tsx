  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Дневной отчёт</h1>
        <div className="flex items-center gap-2">
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#151720] px-3 py-2 text-sm outline-none" />
          <button
            onClick={exportExcel}
      ...
      {isLoading || !data ? (
        ...
        <>
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#151720] overflow-x-auto">
            <div className="px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300 border-b border-gray-100 dark:border-gray-800">
              Проданные товары
            </div>
                ...
                {data.items.map((i) => (
                  <tr key={i.product__sku}>
                    <td className="px-4 py-2">
                      <div className="text-gray-900 dark:text-gray-100">{i.product__name}</div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#151720] p-4">
      ...
      <div className="text-xl font-semibold mt-1 tabular-nums text-gray-900 dark:text-gray-100">{value}</div>
    </div>
  )
}
