            ...
            {docs?.map((d) => (
              <tr key={d.id}>
                ...
                <td className="px-4 py-2.5 text-right">
                  {d.status === 'draft' && (
                    <button onClick={() => setConfirmDoc(d)} className="text-xs font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg px-2.5 py-1.5">Провести</button>
                  )}
                </td>
              </tr>
            ))}
