...
// По умолчанию запросы идут на тот же домен: Django отдаёт и API, и собранный
// фронтенд, а локально dev-сервер Vite проксирует /api на 127.0.0.1:8000.
// VITE_API_URL нужен только если API вынесен на отдельный домен. Vite
// подставляет её на этапе сборки, поэтому после изменения нужен пересбор.
